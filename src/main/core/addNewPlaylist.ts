...
const createNewPlaylist = async (name: string, songIds?: string[], artworkPath?: string) => {
  try {
    const buffer = artworkPath ? await generateLocalArtworkBuffer(artworkPath) : undefined;

    const { playlist: newPlaylist, artworks: newArtworks } = await db.transaction(async (trx) => {
      // Only create artworks if we have an artwork path
      const artworks = buffer ? await storeArtworks('playlist', buffer, trx) : [];
