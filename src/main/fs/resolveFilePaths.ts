...
export const parseSongArtworks = (
  ...
  if (isArtworkAvailable) {
    ...
    const anyImage = artworks[0];

    const mainImage = highResImage || anyImage;
    const optImage = lowResImage || anyImage;

    if (mainImage) {
      // Use simple concatenation for absolute paths to avoid joinPath mangling the protocol
      return {
        isDefaultArtwork: !isArtworkAvailable,
        artworkPath: FILE_URL + mainImage.path + timestampStr,
        optimizedArtworkPath: FILE_URL + optImage.path + timestampStr
      };
    }
  }
...
export const parseArtistArtworks = (
  ...
  if (isArtworkAvailable) {
    ...
    const imageToUse = highResImage || anyImage;

    if (imageToUse) {
      // Use simple concatenation for absolute paths to avoid joinPath mangling the protocol
      const artworkPath = FILE_URL + imageToUse.path + timestampStr;
      return {
        isDefaultArtwork: !isArtworkAvailable,
        artworkPath,
