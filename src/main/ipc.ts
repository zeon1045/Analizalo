...
export function initializeIPC(mainWindow: BrowserWindow, abortSignal: AbortSignal) {
  if (mainWindow) {
    ...
    ipcMain.handle('app/getMusicDownloadsFolder', async () => {
      const settings = await getUserSettings();
      if (settings.musicDownloadsFolder) {
        return settings.musicDownloadsFolder;
      }
      // Default to app's Downloads folder
      const defaultFolder = path.join(app.getPath('userData'), 'Downloads');
    ...
    ipcMain.handle('app/searchOnline', (_, query: string, filter?: SearchFilter) => searchOnline(query, filter));

    ipcMain.handle('app/getOnlineSuggestions', (_, query: string) => getSuggestions(query));

    ipcMain.handle('app/downloadSong', (_, url: string, title: string, artist: string, album: string, artworkUrl?: string) =>
      downloadSong(url, title, artist, album, artworkUrl)
    );
    ...
    // Prefetch single song (for hover pre-loading)
    ipcMain.on('app/prefetchSong', (_, videoId: string) => {
      prefetchVideos([videoId]);
    });

    ipcMain.on('app/clearPrefetchQueue', () => {
      clearPrefetchQueue();
    });

    // Update protected song IDs (songs in playlists that should NEVER be deleted from cache)
    ipcMain.on('app/updateProtectedSongIds', (_, songIds: string[]) => {
      updateProtectedSongIds(songIds);
    });

    // Get current protected song IDs
    ipcMain.handle('app/getProtectedSongIds', () => getProtectedSongIds());

    // Get playlist preset icon path
    ipcMain.handle('app/getPlaylistPresetIconPath', (_, iconName: string) => {
      // In development, resources are in the project root
      // In production, they're in app.getPath('exe')/../resources
      const resourcesPath = IS_DEVELOPMENT
        ? path.join(app.getAppPath(), 'resources', 'playlist-icons')
        : path.join(process.resourcesPath, 'playlist-icons');
      ...
      logger.warn(`Playlist preset icon not found: ${iconName}`, { iconPath });
      return null;
    });
  }
}
