...
const settings = {
  ...
  updateMusicDownloadsFolder: (location: string): Promise<void> =>
    ipcRenderer.invoke('app/saveUserSettings', { musicDownloadsFolder: location }),
  getMusicDownloadsFolder: (): Promise<string> =>
    ipcRenderer.invoke('app/getMusicDownloadsFolder'),
...
const playlistsData = {
  ...
  getPlaylistPresetIconPath: (iconName: string): Promise<string | null> =>
    ipcRenderer.invoke('app/getPlaylistPresetIconPath', iconName)
};
...
const onlineSearch = {
  search: (query: string, filter: 'songs' | 'videos' | 'albums' | 'artists' | 'playlists' = 'songs'): Promise<any[]> => 
    ipcRenderer.invoke('app/searchOnline', query, filter),
  getSuggestions: (query: string): Promise<string[]> => 
    ipcRenderer.invoke('app/getOnlineSuggestions', query),
  download: (
    url: string,
    title: string,
    artist: string,
  ...
  updateProtectedSongIds: (songIds: string[]): void => {
    ipcRenderer.send('app/updateProtectedSongIds', songIds);
  },
  // Get current protected song IDs
  getProtectedSongIds: (): Promise<string[]> =>
    ipcRenderer.invoke('app/getProtectedSongIds')
};
