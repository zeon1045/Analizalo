...
const CurrentlyPlayingSongInfoContainer = () => {
  const currentSongData =
    useStore(store, (state) => state.currentSongData) ?? ({} as AudioPlayerData);
    ...
    return [
      {
        ...
        handlerFunction: () => {
          if (!isKnownSource) {
            addNewNotifications([{
              id: 'playlist-online-song',
              content: 'Para agregar a playlist, primero descarga la canción',
              iconName: 'info',
              type: 'DEFAULT'
            }]);
            return;
  ...
  return (
    <div className="current-playing-song-info-container grid w-full max-w-full grid-cols-[6rem_minmax(0,1fr)_auto] items-center gap-2 lg:grid-cols-[minmax(0,1fr)_auto]">
      <div
        className="song-cover-container relative flex aspect-square h-full items-center justify-center overflow-hidden p-2 lg:hidden"
        id="currentSongCover"
      >
        <Img
        ...
        {currentSongData.isLoading && (
          <div className="absolute inset-2 flex items-center justify-center rounded-lg bg-black/50">
            <span className="material-icons-round animate-spin text-3xl text-white">refresh</span>
          </div>
        )}
      </div>
      ...
      {/* 3-dot menu button */}
      {currentSongData.songId && (
        <div className="song-menu-btn flex items-center justify-center px-2">
          <button
            className="p-2 rounded-full hover:bg-background-color-2 dark:hover:bg-dark-background-color-2 text-font-color-black/60 dark:text-font-color-white/60 hover:text-font-color-black dark:hover:text-font-color-white transition-colors"
            onClick={(e) => {
              e.stopPropagation();
              updateContextMenuData(
                true,
