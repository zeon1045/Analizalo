...
export function useAppLifecycle(dependencies: AppLifecycleDependencies): void {
    ...
    window.api.audioLibraryControls
      ...
      .then((startUpSongData) => {
        if (startUpSongData) {
          ...
          // Skip non-numeric IDs (online/YouTube songs) - they can't be restored from database
          const songId = playback.currentSong.songId;
          if (isNaN(Number(songId))) {
            console.log('[useAppLifecycle] Skipping restore for online song:', songId);
            return undefined;
          }
  ...
  useEffect(() => {
    ...
    const unsubscribeUpNext = playerQueue.on('positionChange', async () => {
      ...
      if (nextSongId) {
        // Skip fetching for non-numeric IDs (online/YouTube songs)
        const numericId = Number(nextSongId);
        if (isNaN(numericId)) {
          console.log('Skipping up next fetch for online song:', nextSongId);
