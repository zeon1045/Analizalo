...
class AudioPlayer extends EventEmitter {
  ...
  private setupQueueIntegration() {
    ...
    this.queue.on('positionChange', () => {
      ...
      if (songId) {
        // Skip non-numeric IDs (online/YouTube songs) - they're handled separately via playOnlineSong
        if (isNaN(Number(songId))) {
          console.log('[AudioPlayer.positionChange] Skipping non-numeric ID (online song):', songId);
  ...
  private async loadSong(
    ...
    try {
      ...
      if (songData.path.startsWith('http')) {
  ...
  async playSongById(
    ...
    try {
      ...
      // Skip non-numeric IDs (online/YouTube songs) - they can't be fetched from database
      if (isNaN(Number(songId))) {
        console.log('[AudioPlayer.playSongById] Skipping non-numeric ID (online song):', songId);
        return;
      }
  ...
  // ========== LOADING STATE (for online songs) ==========

  /**
   * Sets a loading/preview state in the UI before the actual stream is ready.
   * Shows song info immediately while fetching the stream URL.
   * @param previewData - The song data to display (without actual audio path)
   */
  setLoadingState(previewData: AudioPlayerData): void {
    console.log('[AudioPlayer.setLoadingState]', { songId: previewData.songId, title: previewData.title });
    
    // Dispatch to store to show song info in player UI
    dispatch({ 
    ...
    this.emit('loadingStateChange', { isLoading: true, songData: previewData });
  }

  /**
   * Clears the loading state (used when loading fails).
   */
  clearLoadingState(): void {
    console.log('[AudioPlayer.clearLoadingState]');
    
    // Clear current song data or reset loading flag
    dispatch({ 
      type: 'CURRENT_SONG_DATA_CHANGE', 
      data: null 
    });
    
    this.emit('loadingStateChange', { isLoading: false, songData: null });
  }

  /**
   * Plays an online song directly.
  ...
  async skipForward(reason: SongSkipReason = 'USER_SKIP'): Promise<void> {
    ...
    if (this.queue.hasNext) {
      ...
      // Queue is empty - emit event so external handlers (online playlists) can respond
      console.log('[AudioPlayer.skipForward] Queue is empty, emitting skipForwardRequest');
      this.emit('skipForwardRequest', { reason });
    }
  ...
  skipBackward(): void {
    ...
    if (this.queue.currentSongId !== null) {
      ...
      // Queue is empty - emit event so external handlers (online playlists) can respond
      console.log('[AudioPlayer.skipBackward] Queue is empty, emitting skipBackwardRequest');
      this.emit('skipBackwardRequest');
    }
  }
